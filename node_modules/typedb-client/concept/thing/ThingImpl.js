...
(function (ThingImpl) {
    class Remote extends dependencies_internal_1.ConceptImpl.Remote {
        ...
        getHas(annotationOrAttrTypeOrAttrTypes) {
            let isSingleAttrType = false;
            let request;
            if (typeof annotationOrAttrTypeOrAttrTypes === "undefined") {
                request = RequestBuilder_1.RequestBuilder.Thing.getHasReqByAnnotations(this.iid, []);
            }
            else if (Array.isArray(annotationOrAttrTypeOrAttrTypes)) {
                const asArray = annotationOrAttrTypeOrAttrTypes;
                if (asArray.length == 0 || asArray[0] instanceof dependencies_internal_1.AttributeTypeImpl) {
                    request = RequestBuilder_1.RequestBuilder.Thing.getHasByTypeReq(this.iid, annotationOrAttrTypeOrAttrTypes.map(attrType => ThingType_1.ThingType.proto(attrType)));
                }
                else {
                    request = RequestBuilder_1.RequestBuilder.Thing.getHasReqByAnnotations(this.iid, annotationOrAttrTypeOrAttrTypes.map(annotation => Annotation.proto(annotation)));
                }
            }
            else {
                request = RequestBuilder_1.RequestBuilder.Thing.getHasByTypeReq(this.iid, [ThingType_1.ThingType.proto(annotationOrAttrTypeOrAttrTypes)]);
                isSingleAttrType = true;
            }
            const attributes = this.stream(request).flatMap((resPart) => Stream_1.Stream.array(resPart.getThingGetHasResPart().getAttributesList())).map((attrProto) => dependencies_internal_1.AttributeImpl.of(attrProto));
            if (isSingleAttrType) {
                const arg = annotationOrAttrTypeOrAttrTypes;
                if (arg.isBoolean())
                    return attributes;
                else if (arg.isLong())
                    return attributes;
